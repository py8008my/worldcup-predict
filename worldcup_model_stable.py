#!/usr/bin/env python3
"""
世界杯稳健预测模型 v3.5-stable
==============================
数据源：体彩官方API(webapi.sporttery.cn) + 本地知识库(knowledge_base.json)
玩法覆盖：胜平负 / 让球胜平负 / 总进球 / 半全场 / 混合过关（不含比分）
v3.5-stable：博冷模型派生稳健版
  - 概率优先：第一组目标P≥25%（vs博冷10%）
  - 排除比分：稳健策略不碰比分，专注总进球+胜平负+半全场
  - 赔率约束：单组合赔率≤100倍
  - 第二组高概率回本：P≥40% + 赔率≥2.0
  - 复式投注 + 木桶原则 + 竞彩规则合规
"""
...
def _pick_best_bets_per_match(scored_match, top_per_play=3):
    """
    每场比赛的最优投注选择（稳健型：排除比分）
    ==========================================
    稳健策略不碰比分，专注总进球+胜平负+让球+半全场。
    每个玩法取EV最高的top_per_play个候选。
    """
    recs = scored_match.get('recs', [])
    
    # 稳健型：排除比分玩法
    recs = [r for r in recs if r.get('play', '') != '比分']
    ...
    for r in recs:
        ...
        # 赔率过滤（稳健型：上限100倍）
        if play in ('胜平负', '总进球'):
            if odds < 2.5 or odds > 100:
                continue
        elif play in ('半全场', '让球胜平负'):
...
def _gen_smart_plan(scored):
    """
    稳健组合方案 v3.5-stable — 概率优先 + 复式投注 + 双组策略
    ==========================================================
    与博冷版核心差异：
    ① 第一组目标P≥25%（博冷10%），赔率≤100倍
    ② 第二组高概率回本：P≥40%，赔率≥2.0
    ③ 候选池排除比分，专注总进球+胜平负+半全场+让球
    ④ 排序按概率优先（非EV优先）
    """
    ...
    G1_TARGET_PROB = 0.25  # 第一组目标P≥25%
    G1_MAX_ODDS = 100      # 第一组赔率上限
    G2_TARGET_PROB = 0.40  # 第二组目标P≥40%
    G2_MIN_ODDS = 2.0      # 第二组赔率下限
    
    # 按冷门评分排序，取TOP比赛
    top_matches = sorted(scored, key=lambda x: -x['total'])[:6]
    
    # 每场比赛的候选投注项（排除比分，每个玩法TOP3）
    match_candidates = {}
    ...
    if len(match_candidates) < 2:
        return {'label': '⚠️ 稳健方案', 'parts': [], 'total_cost': 0,
                'note': '可投注比赛不足2场'}
    
    mids = list(match_candidates.keys())
    
    # 木桶原则（稳健型不碰比分，关数上限更宽松）
    PLAY_MAX_GUAN = {
        '半全场': 4, '总进球': 6,
        '胜平负': 8, '让球胜平负': 8,
    }
    ...
    for guan_count in [2, 3, 4]:
        ...
        for selected_mids in it.combinations(mids, guan_count):
            ...
            for combo in it.product(*selection_lists):
                ...
                if prob_prod < 0.01:
                    continue
                if odds_prod > G1_MAX_ODDS * 2:  # 允许第二组略宽松
                    continue
    ...
    for mid in mids:
        for s in mid_selections[mid]:
            if s['count'] == 1:
                b = s['bets'][0]
                if b['odds'] <= G1_MAX_ODDS:
                    all_schemes.append(({mid: [b]}, b['ev'], b['odds'], b['prob'], 1, 1))
    
    if not all_schemes:
        return {'label': '⚠️ 稳健方案', 'parts': [], 'total_cost': 0, 'note': '无有效组合'}
    ...
    # ── 第一组（稳健博冷）：P≥25%，赔率≤100倍，预算6-8元 ──
    # 排序：概率降序（稳健优先看概率）
    g1_candidates = [s for s in all_schemes 
                     if s[3] >= G1_TARGET_PROB and s[2] <= G1_MAX_ODDS and 3 <= s[4] <= 4]
    g1_candidates.sort(key=lambda x: -x[3])  # 按P降序
    
    if g1_candidates:
        groups, ev, odds, prob, notes, gc = g1_candidates[0]
        used_keys.add(scheme_key(groups))
        p1 = make_part(groups, ev, odds, prob, notes, gc, ' 稳健')
        plan_parts.append(p1)
        used_notes += notes
    else:
        # 放宽条件：P≥20%，notes≤4
        fallback = [s for s in all_schemes if s[3] >= 0.20 and s[4] <= 4]
        fallback.sort(key=lambda x: -x[3])
        if fallback:
            groups, ev, odds, prob, notes, gc = fallback[0]
            used_keys.add(scheme_key(groups))
            p1 = make_part(groups, ev, odds, prob, notes, gc, ' 稳健')
            plan_parts.append(p1)
            used_notes += notes
    
    # ── 第二组（高概率回本）：P≥40%，赔率≥2.0 ──
    remaining = MAX_NOTES - used_notes
    g2_candidates = []
    for groups, ev, odds, prob, notes, gc in all_schemes:
        ...
        if notes > remaining:
            continue
        if prob >= G2_TARGET_PROB and odds >= G2_MIN_ODDS:
            # 评分：概率优先
            score = prob * _m.log(max(odds, 2.0), 2)
            g2_candidates.append((score, groups, ev, odds, prob, notes, gc))
    
    g2_candidates.sort(key=lambda x: -x[0])
    
    if g2_candidates:
        _, groups, ev, odds, prob, notes, gc = g2_candidates[0]
        used_keys.add(scheme_key(groups))
        ...
        for groups, ev, odds, prob, notes, gc in all_schemes:
            ...
            if notes > remaining:
                continue
            if prob >= 0.30 and odds >= 2.0:
                score = prob * _m.log(max(odds, 2.0), 2)
                fallback2.append((score, groups, ev, odds, prob, notes, gc))
        fallback2.sort(key=lambda x: -x[0])
        if fallback2:
            _, groups, ev, odds, prob, notes, gc = fallback2[0]
            used_keys.add(scheme_key(groups))
            p2 = make_part(groups, ev, odds, prob, notes, gc, ' 回本')
            plan_parts.append(p2)
            used_notes += notes
    ...
    avg_prob = sum(
        float(p.get('note', '').split('P=')[1].split('%')[0]) / 100 
        for p in plan_parts if 'P=' in p.get('note', '')
    ) / max(len(plan_parts), 1)
    ...
    return {
        ...
        'note': f"概率优先 | {len(plan_parts)}组/{total_cost}元 | 最高EV={max_ev:.2f} | 最高倍={max_odds}x"
    }
...
def gen_plan(scored):
    ...
    smart['compliance'] = {
        ...
        'rule_note': '稳健型-排除比分 | 同场不同玩法不可混串 | 木桶原则关数限制 | 复式投注'
    }
...
def print_analysis(scored,kb):
    print("="*80)
    print(f"   🌍 世界杯稳健预测 v3.5-stable | {datetime.now().strftime('%m-%d %H:%M')} | 知识库:{len(kb.get('matches',[]))}场/{len(kb.get('teams',{}))}队")
    print("="*80)
...
def print_plan(plan):
    print("="*80);print("        🛡️ 今日稳健方案 v3.5-stable");print("="*80)
    ...
    print(f"\n  💰 总投入:{total_all}元 | ⚠️ 稳健型 概率优先 复式投注 | 理性投注！");print("="*80)
